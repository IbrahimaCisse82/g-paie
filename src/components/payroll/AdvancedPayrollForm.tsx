...
import { calculateAdvancedSalaryElements } from '@/utils/payrollCalculations';
import { Calculator } from 'lucide-react';

export const AdvancedPayrollForm = () => {
  ...
  const calculateMutation = useMutation({
    mutationFn: async () => {
      ...
      return calculateAdvancedSalaryElements(
        formData.employe_id,
        Number(formData.salaire_brut),
        Number(formData.sur_salaire),
        formData.mois,
        Number(formData.annee),
        {
          ...
          prime_anciennete_rate: Number(formData.prime_anciennete_rate) || 0,
    ...
    onSuccess: (result) => {
      toast({
        title: "Succès",
        description: `Calcul de paie effectué avec succès. Salaire net: ${result.salaire_net?.toLocaleString()} FCFA`,
      });
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Calcul Avancé de Paie</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="salaire_brut">Salaire de Base (FCFA) *</Label>
              <Input
                id="salaire_brut"
                type="number"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData(prev => ({...prev, sur_salaire: e.target.value}))}
                placeholder="0"
              />
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="prime_anciennete_rate">Taux Prime d'Ancienneté (%)</Label>
              <Input
                id="prime_anciennete_rate"
                type="number"
                step="0.1"
                value={formData.prime_anciennete_rate}
                onChange={(e) => setFormData(prev => ({...prev, prime_anciennete_rate: e.target.value}))}
                placeholder="0"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData(prev => ({...prev, emploi: e.target.value}))}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData(prev => ({...prev, contrat: e.target.value}))}
                placeholder="Ex: CDI"
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor="situation_contrat">Situation du Contrat</Label>
              <Input
                id="situation_contrat"
                value={formData.situation_contrat}
                onChange={(e) => setFormData(prev => ({...prev, situation_contrat: e.target.value}))}
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              <li>• IPRES RG : 5,6% du salaire brut total</li>
              <li>• CSS Allocations Familiales : 7% (charges patronales)</li>
              <li>• CSS Accident de Travail : 5% (charges patronales)</li>
              <li>• CFCE : 3% (charges patronales)</li>
              <li>• Retenue TRIMF : 800 FCFA (fixe)</li>
            </ul>
          </div>
          ...
          >
            {calculateMutation.isPending ? 'Calcul en cours...' : 'Calculer la Paie Avancée'}
          </Button>
        </form>
      </CardContent>
    </Card>
